...
function findSatAbove() {
    ...
    if (testLatint == false) {
        errmsg = errmsg + "Lat must be a number <br />";
    }
    if (testLngint == false) {
        errmsg = errmsg + "Lng must be a number <br />";
    }
    if (testLat == false) {
        errmsg = errmsg + "Lat must be a value between -90 &amp; 90 <br />";
    }
    if (testLng == false) {
        errmsg = errmsg + "Lng must be a  value between -180 &amp; 180 <br />";
    }
    ...
    if (errmsg != "") {
        ...
        deleteMarkers();
        map.setCenter({
            lat: parseFloat(sLat.value),
            lng: parseFloat(sLng.value)
        });
        let data = "apiKey=" + apiKey;
...
function animateSat(satPos) {
    ...
    map.setCenter({
        lat: parseFloat(satPos.positions[i].satlatitude),
        lng: parseFloat(satPos.positions[i].satlongitude)
    });
    ...
    for (var j = 0; j < 300; j++) {
        flightPlanCoordinates.push({
            lat: parseFloat(satPos.positions[j].satlatitude),
            lng: parseFloat(satPos.positions[j].satlongitude)
        });
        console.log("c");
    }
...
function satDetail(searchsatid, key) {
    ...
    map.setCenter({
        lat: parseFloat(resultObject.satlat),
        lng: parseFloat(resultObject.satlng)
    });
...
function showAddDetail(addDetail) {
    ...
    function setMarkers(thisSat) {
        ...
        var marker = new google.maps.Marker({
            ...
            position: {
                lat: parseFloat(thisSat.satlat),
                lng: parseFloat(thisSat.satlng)
            },
            map: map,
    ...
    } + addDetail["Longitude of GEO (degrees)"] + "&deg;<br />" +
        "Perigee: " + addDetail["Perigee (km)"] + " km<br />" +
        "Apogee: " + addDetail["Apogee (km)"] + " km<br />" +
        "Eccentricity: " + addDetail["Eccentricity"] + "<br />" +
        "Inclination: " + addDetail["Inclination (degrees)"] + "&deg;<br />" +
